        ...
        if self.drop_optout:
            if year == 19:
                ...
                student_data = pd.read_csv(
                    f"Zone_Generation/Zone_Data/student_1920.csv", low_memory=False)
                student_data = student_data.dropna(subset=["enrolled_idschool"])

            else:
                student_data = pd.read_csv(
                    # f"Zone_Data/drop_optout_{year}{year + 1}.csv", low_memory=False
                    f"Zone_Generation/Zone_Data/enrolled_{year}{year + 1}.csv", low_memory = False)
        else:
            student_data = pd.read_csv(
                f"Zone_Generation/Zone_Data/student_{year}{year + 1}.csv", low_memory=False)
